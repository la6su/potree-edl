...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  if (volume.region) {
    ...
    const sphere = new Sphere(new Vector3(volume.sphere[0], volume.sphere[1], volume.sphere[2]), volume.sphere[3]);
    return {
      sphere
    };
  } else {
...
class $3dTilesIndex {
  ...
  _recurse(node, baseURL, parent) {
    ...
    indexedNode.transformMatrix = node.transform ? new Matrix4().fromArray(node.transform) : identity;
    ...
    if (parent && parent.worldFromLocalTransform != null) {
      if (indexedNode.transform) {
        indexedNode.worldFromLocalTransform = new Matrix4().multiplyMatrices(parent.worldFromLocalTransform, indexedNode.transformMatrix);
      } else {
    ...
    indexedNode.viewerRequestVolumeObject = indexedNode.viewerRequestVolume ? getBox(indexedNode.viewerRequestVolume, this._inverseTileTransform) : undefined;
    indexedNode.boundingVolumeObject = getBox(indexedNode.boundingVolume, this._inverseTileTransform);
