...
class MemoryUsage extends ChartPanel {
  ...
  updateValues() {
    ...
    pushTrim(this.textures.data, {
      x: frame,
      y: this.memory.textures
    }, MAX_DATA_POINTS);
    pushTrim(this.geometries.data, {
      x: frame,
      y: this.memory.geometries
    }, MAX_DATA_POINTS);
    pushTrim(this.programs.data, {
      x: frame,
      y: this.instance.renderer.info.programs?.length ?? 0
    }, MAX_DATA_POINTS);
    pushTrim(this.renderTargets.data, {
      x: frame,
      y: GlobalRenderTargetPool.count
    }, MAX_DATA_POINTS);
